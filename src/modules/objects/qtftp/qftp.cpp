...
//    qDebug("QFtpPI state: %d [processReply() intermediate]", state);
	...
	{
		if(currentCmd.startsWith(QLatin1String("USER ")) && pendingCommands.count() > 0 && pendingCommands.first().startsWith(QLatin1String("PASS ")))
		{
...
void QFtp::_q_piFinished(const QString & a)
{
	d->_q_piFinished(a);
}
void QFtp::_q_piError(int a, const QString & b)
{
	d->_q_piError(a, b);
}
void QFtp::_q_piConnectState(int a)
{
	d->_q_piConnectState(a);
}
void QFtp::_q_piFtpReply(int a, const QString & b)
{
	d->_q_piFtpReply(a, b);
}
