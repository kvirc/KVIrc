...
void AliasEditorWidget::aliasRefresh(const QString &szName)
{
	...
	if(
		QMessageBox::warning(0,__tr2qs_ctx("Overwrite Current Alias - KVIrc","editor"),
...
void AliasEditorWidget::exportSelectionInSinglesFiles(KviPointerList<AliasEditorTreeWidgetItem> *l)
{
	...
	{
		g_pAliasEditorModule->lock();
		QMessageBox::warning(this,__tr2qs_ctx("Warning While Exporting - KVIrc","editor"),__tr2qs_ctx("There is no selection!","editor"),__tr2qs_ctx("OK","editor"));
		g_pAliasEditorModule->unlock();
		return;
	}
	g_pAliasEditorModule->lock();
...
void AliasEditorWidget::exportAliases(bool bSelectedOnly,bool bSingleFiles)
{
	...
	{
		g_pAliasEditorModule->lock();
		QMessageBox::warning(this,__tr2qs_ctx("Write to File Failed - KVIrc","editor"),__tr2qs_ctx("Unable to write to the aliases file.","editor"),__tr2qs_ctx("OK","editor"));
		g_pAliasEditorModule->unlock();
	}
}
...
QString AliasEditorWidget::askForAliasName(const QString &szAction,const QString &szText,const QString &szInitialText)
{
	...
	while(szNewName.isEmpty())
	{
		...
		if(!re.exactMatch(szNewName))
		{
			g_pAliasEditorModule->lock();
			QMessageBox::information(this,
				__tr2qs_ctx("Invalid Name Notice - KVIrc","editor"),
...
{
	QString szNewName = askForAliasName(__tr2qs_ctx("Choose a Name - KVIrc","editor"),__tr2qs_ctx("Please enter the name for the new alias","editor"),"myfunction");
	if(szNewName.isEmpty())return;
...
{
	QString szName = askForNamespaceName(__tr2qs_ctx("Choose a Name - KVIrc","editor"),__tr2qs_ctx("Please enter the name for the new namespace","editor"),"mynamespace");
