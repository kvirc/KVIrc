...
namespace KviKvsCoreFunctions
{
	...
	*/
	...
	{
		KVSCF_pRetBuffer->setBoolean(g_pApp->topmostConnectedConsole() ? true : false);
		return true;
	}
	...
	*/
	...
	KVSCF(isMeHalfOp)
	{
		...
		KVSCF_PARAMETERS_BEGIN
			KVSCF_PARAMETER("windowId",KVS_PT_STRING,KVS_PF_OPTIONAL,winId)
		KVSCF_PARAMETERS_END
		KviWindow * wnd;
		if(winId.isEmpty())wnd = KVSCF_pContext->window();
		else {
			wnd = g_pApp->findWindow(winId.utf8().data());
			if(!wnd)
			{
				KVSCF_pContext->warning(__tr2qs("Window with ID '%s' not found"),winId.utf8().data());
				KVSCF_pRetBuffer->setNothing();
				return true;
			}
		}
		...
		KVSCF_pRetBuffer->setBoolean(((KviChannel *)wnd)->isMeHalfOp());
		return true;
	}
	...
	*/
	...
	KVSCF(isMeOp)
	{
		...
		KVSCF_pRetBuffer->setBoolean(((KviChannel *)wnd)->isMeOp());
		return true;
	}
	...
	*/
	...
	KVSCF(isMeUserOp)
	{
		...
		KVSCF_pRetBuffer->setBoolean(((KviChannel *)wnd)->isMeUserOp());
		return true;
	}
	...
	*/
	...
	KVSCF(isMeVoice)
	{
		...
		KVSCF_pRetBuffer->setBoolean(((KviChannel *)wnd)->isMeVoice());
		return true;
	}
