...
void KviIrcView::stopLogging()
{
	...
	{
		QString szLogEnd;
		KviQString::sprintf(szLogEnd,__tr2qs("### Log session terminated at %s ###"),QDateTime::currentDateTime().toString().toUtf8().data());
...
bool KviIrcView::startLogging(const QString& fname,bool bPrependCurBuffer)
{
	...
	QString szLogStart;
	KviQString::sprintf(szLogStart,__tr2qs("### Log session started at %s ###"),QDateTime::currentDateTime().toString().toUtf8().data());
	add2Log(szLogStart);
