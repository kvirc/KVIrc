...
void KviChannel::getConfigOldGroupName(QString &buf)
{
	buf = windowName();
}
...
void KviChannel::ownAction(const QString &buffer)
{
	...
	const char * d = szData.data();
	if(!d)return;
	if(KVS_TRIGGER_EVENT_2_HALTED(KviEvent_OnMeAction,this,szTmpBuffer,szName))return;
	if(!connection()->sendFmtData("PRIVMSG %s :%cACTION %s%c",connection()->encodeText(szName).data(),0x01,d,0x01))return;
