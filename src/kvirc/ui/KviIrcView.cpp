				...
				if(checkSelectionBlock(pCurTextLine,i))
				{
					...
					if(wdth == 0)
					{
						...
						if(i < (pCurTextLine->iBlockCount - 1))
						{
							...
							if(pCurTextLine->pBlocks[i+1].pChunk == 0)
...
void KviIrcView::chooseBackground()
{
	...
	if(p.isNull())
	{
		QMessageBox::information(this,__tr2qs("Invalid Image Notice - KVIrc"),__tr2qs("Failed to load the selected image!"),__tr2qs("OK"));
		return;
	}
