...
void KviDefaultScriptManager::restore()
{
	...
	{
		QMessageBox::warning(0,__tr2qs("Restore Default Script - KVIrc"),szError,QMessageBox::Ok,QMessageBox::NoButton,QMessageBox::NoButton);
		return;
	}

	if(m_bNoNeedToRestore)
	{
		if(QMessageBox::warning(0,__tr2qs("Restore Default Script - KVIrc"),szError,QMessageBox::Yes, QMessageBox::No) != QMessageBox::Yes)
			return;
	}
